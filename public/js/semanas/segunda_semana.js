      ...
      fetchedData.forEach((data) => {
        ...
        tableBody.innerHTML += `
            <tr>
              ...
              <td>${data.piano || ""}</td>
              <td class="trumpet_body">${data.trumpet || ""}</td>
              <td class="acousticGuitar2_body">${
                data.acousticGuitar2 || ""
              }</td>
              <td class="electricGuitar2_body">${
                data.electricGuitar2 || ""
              }</td>
              <td>${data.direccion || ""}</td>
              <td>${data.director || ""}</td>
              <td>${data.voz1 || ""}</td>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all(promises)
    .then((dataArray) => {
      const allVoz4Empty = dataArray.every(
        (data) => !data.trumpet || data.trumpet.trim() === ""
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all(promises)
    .then((dataArray) => {
      const allVoz4Empty = dataArray.every(
        (data) => !data.acousticGuitar2 || data.acousticGuitar2.trim() === ""
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all(promises)
    .then((dataArray) => {
      const allVoz4Empty = dataArray.every(
        (data) => !data.electricGuitar2 || data.electricGuitar2.trim() === ""
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all(promises)
    .then((dataArray) => {
      // Verificar si todos los apartados voz4 están vacíos para todas las fechas seleccionadas
      const allSong4Empty = dataArray.every(
        (data) => !data.song4 || data.song4.trim() === ""
      );
